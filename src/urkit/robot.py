...
class URRobot:
    ...
    def interruptible_move_to(
        ...
        *,
        ...
        interrupted with ``KeyboardInterrupt`` (Ctrl+C). The robot
        approaches the target using proportional control and stops when
        within 1mm. Press Ctrl+C to abort and return to the menu.
        ...
        Example:
            >>> robot.interruptible_move_to("home")
            >>> robot.interruptible_move_to("pick", vel=0.5)  # Ctrl+C to abort
        """
        self._check_connection()
        self._disable_freedrive_guard()
        ...
        if linear:
            self._motion.move_to_interruptible(pose, vel=vel, acc=acc)
        else:
            # For joint moves, fall back to blocking moveJ
            # (interruptible joint control loop is more complex)
            joints = self.inverse_kinematics(pose)
            self._motion.movej(joints, vel=vel, acc=acc)
